                    ...
                    DetailMode::Read => {
                        if let Some(item) = &selected_item {
                            ...
                            if has_totp_secret && totp_last_poll.elapsed() >= TOTP_POLL_INTERVAL {
                                ...
                                // on commit 1d6c5ab: any error blanks
                                // `totp_code` rather than leaving a stale
                                // code rendering under a countdown that
                                ...
                                // configured" (`Ok(None)`) already blanks the
                                // field with no reauth, and still does.
                                let poll_result = cache.bridge().get_totp(&item.id);
                                let error = apply_totp_poll_result(poll_result, &mut totp_code);
...
/// Applies one `get_totp` poll result to `totp_code`, returning the error
...
/// `eframe` context.
///
/// This is the regression fix for review Important 1 on commit 1d6c5ab: that
/// commit changed the poll's `Err` arm to leave `totp_code` untouched, on the
/// reasoning that a transient non-401 failure isn't evidence the code
/// stopped being valid. That reasoning is backwards for TOTP specifically --
/// the code expires within 30 seconds regardless of whether the connection
/// to `bw serve` is healthy, and the pane's `seconds_left` countdown is
/// derived from the wall clock rather than from the fetch, so holding the
/// old code renders it under a countdown that keeps cycling as if the code
/// were still live. There is no failure mode where holding a TOTP code past
/// its window is safer than blanking it, so *any* error blanks `totp_code`
/// -- restoring the pre-1d6c5ab always-assign behaviour -- while a genuine
/// `Ok(None)` ("no TOTP configured") continues to blank it too, exactly as
/// before.
fn apply_totp_poll_result(result: Result<Option<String>, VaultError>, totp_code: &mut Option<String>) -> Option<VaultError> {
    match result {
        Ok(code) => {
            *totp_code = code;
            None
...
mod apply_totp_poll_result_tests {
    // Regression tests for review Important 1 on commit 1d6c5ab: that
    // commit's poll `Err` arm left a stale TOTP code on screen under a
    // countdown that kept ticking as though it were still valid. These pin
    // the fix -- any error blanks the code -- while confirming the genuine
    // "no TOTP configured" case still blanks it too, with no error to report.
    use super::apply_totp_poll_result;
    use crate::vault_bridge::VaultError;

    #[test]
    fn a_successful_fetch_replaces_the_code() {
        let mut totp_code = Some("111111".to_string());

        let error = apply_totp_poll_result(Ok(Some("222222".to_string())), &mut totp_code);

        assert_eq!(totp_code, Some("222222".to_string()));
        assert!(error.is_none());
    }

    #[test]
    fn a_genuine_no_totp_configured_response_blanks_the_code_with_no_error() {
        let mut totp_code = Some("111111".to_string());

        let error = apply_totp_poll_result(Ok(None), &mut totp_code);
    ...
    #[test]
    fn any_error_blanks_a_previously_held_code_instead_of_freezing_it() {
        ...
        let error = apply_totp_poll_result(Err(VaultError::Http("connection reset".to_string())), &mut totp_code);

        assert_eq!(totp_code, None, "a poll error must blank the displayed code, not freeze it");
        assert!(matches!(error, Some(VaultError::Http(_))));
    }

    #[test]
    fn an_unauthorized_error_also_blanks_the_code_and_is_returned() {
        let mut totp_code = Some("111111".to_string());

        let error = apply_totp_poll_result(Err(VaultError::Unauthorized), &mut totp_code);
