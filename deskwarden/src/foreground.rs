...
mod tests {
    ...
    fn every_module_in_this_crate_is_classified_as_opening_windows_or_not() {
        ...
        const OPENS_NO_WINDOW: [&str; 81] = [
            ...
            "record",
            // `receive_history.json`: three fields per imported Send -- a
            // timestamp, a name and the vault item id -- read and written
            ...
            "service_body",
            // Key records, scopes and expiry: serde, a hash and some
            // comparisons. No window, no socket, no clock of its own --
            // `find` is told what time it is.
            "service_keys",
