...
mod startup_window_tests {
    ...
    const BELOW_CUT_RULES: crate::below_cut::WalkRules = crate::below_cut::WalkRules {
        ...
        top_level_item_note: "Every source guard in this file slices at the first test gate and reads only what is ABOVE it, so an item down here is read by none of them: it can open a window off the pinned path, duplicate a call site pinned at exactly one, or reintroduce a construct banned by name, and the suite stays green.",
