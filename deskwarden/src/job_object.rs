...
mod tests {
    ...
    fn the_two_job_bearing_modules_can_start_a_child_only_through_this_one() {
        ...
        assert_eq!(
            ...
            // 16165 bytes, new hash: the crate version moved 0.14.0 -> 0.15.0
            // for the release cut. No dependency changed -- the length is
            // identical because the two version strings are the same width,
            // and only the hash moves. Recomputed with CRLF normalised to LF,
            // and validated against the old pinned pair first.
            // 16165 bytes, new hash: the crate version moved 0.15.0 -> 0.15.1
            // for the point release. No dependency changed; the length is
            // identical because the version strings are the same width.
            // 16165 bytes, new hash: the crate version moved 0.15.1 -> 0.15.2
            // for the point release. No dependency changed.
            // 16165 bytes, new hash: the crate version moved 0.15.2 -> 0.15.3
            // for the point release. No dependency changed.
            // 16165 bytes, new hash: the crate version moved 0.15.3 -> 0.15.4
            // for the point release. No dependency changed.
            // 16165 bytes, new hash: the crate version moved 0.15.4 -> 0.15.5
            // for the point release. No dependency changed.
            // 16165 bytes, new hash: the crate version moved 0.15.5 -> 0.15.6
            // for the point release. No dependency changed.
            // 16165 bytes, new hash: the crate version moved 0.15.6 -> 0.15.7
            // for the point release. No dependency changed.
            // 16165 bytes, new hash: the crate version moved 0.15.7 -> 0.15.8
            // for the point release. No dependency changed.
            // 16165 bytes, new hash: the crate version moved 0.15.8 -> 0.15.9
            // for the point release. No dependency changed.
            // 16166 bytes: the crate version moved 0.15.9 -> 0.15.10, one
            // character wider than every bump before it. No dependency
            // changed; the length moves because "10" is two digits.
            // 16166 bytes, new hash: the crate version moved 0.15.10 ->
            // 0.15.11 for the point release. No dependency changed; both
            // versions are the same width, so only the hash moves.
            // 16166 bytes, new hash: the crate version moved 0.15.11 ->
            // 0.15.12 for the point release. No dependency changed.
            // 16166 bytes, new hash: the crate version moved 0.15.12 ->
            // 0.15.13 for the point release. No dependency changed.
            // 16718 bytes, new hash: the crate version moved 0.15.13 ->
            // 0.15.14-dev, which is how a build from the working tree now
            // tells itself apart from a GitHub release, plus the seven-line
            // comment in `Cargo.toml` that states the rule. That comment is
            // most of the 552 bytes; the version itself is four of them. No
            // dependency changed.
            // 16714 bytes, new hash: the crate version moved 0.15.14-dev ->
            // 0.15.14 for the release, dropping the four-byte suffix the
            // comment above it describes. No dependency changed.
            // 16718 bytes, new hash: the crate version moved 0.15.14 ->
            // 0.15.15-dev, putting the suffix back the commit after the
            // release, which is the rule stated in `Cargo.toml`. No
            // dependency changed.
            // 16714 bytes, new hash: the crate version moved 0.15.15-dev ->
            // 0.15.15 for the release. No dependency changed.
            ...
            // dependency changed.
            //
            // 17484 bytes, new hash: and back to 0.15.17-dev the commit
            // after, per `Cargo.toml`'s rule. No dependency changed.
            //
            // 17480 bytes, new hash: the crate version moved 0.15.17-dev ->
            // 0.15.17 for the release, dropping the four-byte suffix. No
            // dependency changed.
            //
            // 17484 bytes, new hash: and on to 0.15.18-dev the commit after,
            ...
            //
            // **27355 bytes, new hash: the 0.15.20 release, and nothing
            // else.** `version = "0.15.19"` became `"0.15.20"` -- the same
            // seven characters, which is why the LENGTH does not move and
            // the hash is the only half of the pair that notices. That is
            // worth saying out loud: a length-preserving edit is exactly
            // the shape a pin of length alone would miss, and it is why
            // this pair carries a hash at all.
            //
            ...
            // `[build-dependencies]` still reads exactly
            // `winresource = "0.1"` -- the only things this pair exists to
            // notice.
            //
            // Recomputed the way every hop above records -- FNV-1a/64 over
            // the file with CRLF normalised to LF, in a separate
            ...
            // the 0.15.22 release.** `version = "0.15.22"` became `"0.15.23-dev"`.
            //
            // This is the hop the release commit above it promised, and the one the
            // 0.15.20 and 0.15.21 cuts forgot: without it every build from the working
            // tree claims to BE the release it is sitting on top of, which is exactly
            // what the changelog's Unreleased section promises it will not do.
            //
            // Five bytes longer -- `-dev` plus the digit that rolls -- so the length
            // moves with the hash.
            //
            // No dependency was added, removed, re-pointed or re-featured, no
            // `[patch]`/`[replace]`/`[workspace.dependencies]` table appeared, no
            ...
            // crate -- rather than copied out of the failure message. That
            // implementation was first run against the PREVIOUS pinned pair and
            // reproduced (27355, 0xcd71_663f_a7b0_6ba5) exactly, so it is measuring
            // what this test measures.
            // **27355 bytes, new hash: the 0.15.23 release, and nothing else.**
            // `version = "0.15.23-dev"` became `"0.15.23"`. Five bytes shorter, the
            // mirror of the hop above it: that one put the suffix back so a
            // working-tree build would stop claiming to BE a release, and this one
            // takes it off for the build that IS one. The length moves with the hash
            // again.
            //
            // The commit after this one puts `-dev` back, as the hop above it did
            // for 0.15.22 -- the step the 0.15.20 and 0.15.21 cuts both forgot.
            //
            // No dependency was added, removed, re-pointed or re-featured, no
            // `[patch]`/`[replace]`/`[workspace.dependencies]` table appeared, no
            // path or fork appeared, and `[build-dependencies]` still reads exactly
            ...
            // what this test measures.
            // **27355 bytes, new hash: the 0.15.24 release, and nothing else.**
            // `version = "0.15.24-dev"` became `"0.15.24"`. Four bytes shorter, the
            // mirror of the hop above it, and the same pair of hops every release
            // since 0.15.22 has made: off for the build that IS the release, back on
            ...
            // crate -- rather than copied out of the failure message. That
            // implementation was first run against the PREVIOUS pinned pair and
            // reproduced (27359, 0x0893_8927_73e3_6cc1) exactly, so it is measuring
            // what this test measures.
            // **27359 bytes, new hash: the version goes back to a `-dev` suffix after
            // the 0.15.24 release.** `version = "0.15.24"` became `"0.15.25-dev"`.
            //
            // The second half of the pair every release makes, and the half the
            // 0.15.20 and 0.15.21 cuts forgot: without it every build from the
            // working tree claims to BE the release it sits on top of.
            //
            // Four bytes longer, so the length moves with the hash.
            //
            // No dependency was added, removed, re-pointed or re-featured, no
            // `[patch]`/`[replace]`/`[workspace.dependencies]` table appeared, no
            // path or fork appeared, and `[build-dependencies]` still reads exactly
            ...
            // crate -- rather than copied out of the failure message. That
            // implementation was first run against the PREVIOUS pinned pair and
            // reproduced (27355, 0xdabb_dc3a_25d7_6eb7) exactly, so it is measuring
            // what this test measures.
            // **27355 bytes, new hash: the 0.15.25 release, and nothing else.**
            // `version = "0.15.25-dev"` became `"0.15.25"`. Four bytes shorter, the
            // mirror of the hop above it, and the same pair of hops every release
            // since 0.15.22 has made: off for the build that IS the release, back on
            // in the commit after it.
            //
            ...
            // crate -- rather than copied out of the failure message. That
            // implementation was first run against the PREVIOUS pinned pair and
            // reproduced (27359, 0x9f54_39d5_e231_e830) exactly, so it is measuring
            // what this test measures.
            // **27359 bytes, new hash: the version goes back to a `-dev` suffix after
            // the 0.15.25 release.** `version = "0.15.25"` became `"0.15.26-dev"`.
            //
            // The second half of the pair every release makes, and the half the
            // 0.15.20 and 0.15.21 cuts forgot: without it every build from the
            // working tree claims to BE the release it sits on top of, which is
            // exactly what the changelog's Unreleased section promises it will not do.
            //
            // Five bytes longer -- the `-dev` suffix, and the patch digit rolling
            // from 5 to 6 within the same width -- so the length moves with the hash.
            //
            // No dependency was added, removed, re-pointed or re-featured, no
            // `[patch]`/`[replace]`/`[workspace.dependencies]` table appeared, no
            // path or fork appeared, and `[build-dependencies]` still reads exactly
            ...
            // crate -- rather than copied out of the failure message. That
            // implementation was first run against the PREVIOUS pinned pair and
            // reproduced (27355, 0x2231_8183_54a2_368e) exactly, so it is measuring
            // what this test measures.
            // **27355 bytes, new hash: the 0.15.26 release, and nothing else.**
            // `version = "0.15.26-dev"` became `"0.15.26"`. Four bytes shorter -- the
            // suffix and nothing beside it -- so the length moves with the hash.
            //
            // The commit after this one puts `-dev` back, which is the step the
            // 0.15.20 and 0.15.21 cuts both forgot: without it a build from the
            // working tree claims to BE the release it sits on top of.
            //
            // No dependency was added, removed, re-pointed or re-featured, no
            // `[patch]`/`[replace]`/`[workspace.dependencies]` table appeared, no
            ...
            // crate -- rather than copied out of the failure message. That
            // implementation was first run against the PREVIOUS pinned pair and
            // reproduced (27359, 0xae28_778c_c6e4_0837) exactly, so it is measuring
            // what this test measures.
            // **27359 bytes, new hash: the version goes back to a `-dev` suffix after
            // the 0.15.26 release.** `version = "0.15.26"` became `"0.15.27-dev"`.
            // Four bytes longer, the mirror of the hop above it, so a build from the
            // working tree stops claiming to BE the release it sits on top of.
            //
            // No dependency was added, removed, re-pointed or re-featured, no
            // `[patch]`/`[replace]`/`[workspace.dependencies]` table appeared, no
            // path or fork appeared, and `[build-dependencies]` still reads exactly
            ...
            // what this test measures.
            // **28448 bytes, new hash: the 0.15.27 release, and nothing else.**
            // `version = "0.15.27-dev"` became `"0.15.27"`. Four bytes shorter -- the
            // suffix and nothing beside it -- so the length moves with the hash.
            //
            // The commit after this one puts `-dev` back.
            //
            // No dependency was added, removed, re-pointed or re-featured, no
            // `[patch]`/`[replace]`/`[workspace.dependencies]` table appeared, no
            // path or fork appeared, and `[build-dependencies]` still reads exactly
            ...
            // crate -- rather than copied out of the failure message. That
            // implementation was first run against the PREVIOUS pinned pair and
            // reproduced (28452, 0x5e89_fff3_3456_f7b1) exactly, so it is measuring
            // what this test measures.
            // **28452 bytes, new hash: the version goes back to a `-dev` suffix after
            // the 0.15.27 release.** `version = "0.15.27"` became `"0.15.28-dev"`.
            // Four bytes longer, the mirror of the hop above it, so a build from the
            // working tree stops claiming to BE the release it sits on top of.
            //
            // No dependency was added, removed, re-pointed or re-featured, no
            ...
            // crate -- rather than copied out of the failure message. That
            // implementation was first run against the PREVIOUS pinned pair and
            // reproduced (28448, 0xb267_ad6c_296a_55e3) exactly, so it is measuring
            // what this test measures.
            (28452, 0x278a_e1ce_2972_4f1c_u64),
